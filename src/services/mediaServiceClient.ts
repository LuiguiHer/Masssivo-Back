...
export type MediaUploadResult = { bucket: string; key: string; publicUrl: string };
...
export async function uploadFileToMediaService(
  ...
  const d = data as { bucket?: string; key?: string; publicUrl?: string };
  if (!d.bucket || !d.key || !d.publicUrl) throw new Error("Respuesta de almacenamiento inválida");
  return { bucket: d.bucket, key: d.key, publicUrl: d.publicUrl };
}

export async function deleteObjectFromMediaService(baseUrl: string, internalKey: string, key: string): Promise<void> {
  if (!key.trim()) return;
