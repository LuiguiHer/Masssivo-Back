export function canonicalWaId(input: string): string {
...
export function waIdAliases(input: string): string[] {
...
export function waIdGroupKey(input: string): string {
  const base = canonicalWaId(input);
  if (!base) return "";
  // Agrupa 57XXXXXXXXXX con XXXXXXXXXX en una sola conversación.
  if (base.startsWith("57") && base.length === 12) return base.slice(2);
  return base;
}
