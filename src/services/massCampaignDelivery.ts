...
function extractMetaErrorReason(errors: unknown[] | undefined | null): string {
  const arr = Array.isArray(errors) ? errors : [];
  const first = arr[0] as { message?: string; title?: string; code?: number } | undefined;
  const raw =
    String(first?.message ?? first?.title ?? "").trim() ||
    (first?.code != null ? `Código ${first.code}` : "") ||
    JSON.stringify(first ?? {}).slice(0, 280);
  return raw.slice(0, 500);
}

/** Recalcula contadores de campaña a partir de rowResults (única fuente de verdad). */
export function computeDeliveryStatsFromRows(rowResults: RowLike[] | undefined | null): {
  sentCount: number;
  failCount: number;
  ...
  for (const r of rows) {
    ...
    if (!wamid) {
      if (ok === false || r.apiOk === false) failCount += 1;
      continue;
    }
    sentCount += 1;
    if (ok === true || st === "delivered" || st === "read") deliveredCount += 1;
    else if (ok === false || st === "failed") deliveryFailedCount += 1;
    else pendingDeliveryCount += 1;
  }

  return { sentCount, failCount, deliveredCount, deliveryFailedCount, pendingDeliveryCount };
}

/** Prioridad de avance de entrega (Meta webhook); mayor = más informativo que "pending" del cliente. */
function deliveryProgressRank(r: RowLike): number {
  const st = String(r.deliveryStatus ?? "").toLowerCase();
  const wid = String(r.wamid ?? "").trim();
  if (st === "failed" || (r.ok === false && wid)) return 4;
  if (st === "read" || st === "delivered" || r.ok === true) return 3;
  if (st === "sent") return 2;
  if (!st || st === "pending") return 1;
  return 2;
}
 ...
 */
export function mergeIncomingRowResultsWithExisting(
  existingRows: RowLike[] | undefined | null,
  incomingRows: RowLike[],
): RowLike[] {
  const existing = Array.isArray(existingRows) ? existingRows : [];
  const exMap = new Map<number, RowLike>();
  for (const r of existing) {
    const idx = Math.floor(Number(r.rowIndex));
    if (Number.isFinite(idx) && idx >= 1) exMap.set(idx, r);
  }
  const out: RowLike[] = [];
  ...
  for (const inc of incomingRows) {
    ...
    if (rex > rinc) {
      ...
      if (inT && exT && tin > tex) out.push({ ...ex, ...inc });
      else if (inT && !exT) out.push({ ...ex, ...inc });
      else
        out.push({
          ...inc,
          deliveryStatus: ex.deliveryStatus,
  ...
  for (const [idx, ex] of exMap) {
    if (!incomingIndices.has(idx)) out.push(ex);
  }
...
/**
 * Actualiza la fila de campaña que coincide con wamid y recalcula contadores.
 */
...
export async function syncMassCampaignRowFromWebhook(
  ...
  let okBool: boolean | undefined;
  if (statusLower === "failed" || hasErr) okBool = false;
  else if (statusLower === "delivered" || statusLower === "read") okBool = true;

  const reasonTxt = okBool === false ? extractMetaErrorReason(errArr) : undefined;
  ...
  const updatePayload: Record<string, unknown> = { $set: setOps };
  if (Object.keys(unsetOps).length) updatePayload.$unset = unsetOps;
