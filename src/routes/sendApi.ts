...
} from "../services/massCampaignDelivery.js";
...
type LeanCompanyWhatsappConfig = CompanyWhatsappConfigDoc & { _id: Types.ObjectId };
type MessageStartTemplateCfg = {
...
function resolveMetaAppId(cfg: LeanCompanyWhatsappConfig): string {
...
function metaResumableFileType(mimetype: string, originalname: string): string | null {
  const m = (mimetype || "").toLowerCase();
  const ext = (originalname.includes(".") ? originalname.split(".").pop() ?? "" : "").toLowerCase();
  if (m === "image/jpeg" || m === "image/jpg" || ext === "jpg" || ext === "jpeg") return "image/jpeg";
...
function buildInboxTemplateDisplay(components: unknown[] | undefined, footerText: string): InboxTemplateDisplay | undefined {
  const out: InboxTemplateDisplay = {};
  const ft = footerText.trim();
  if (ft) out.footerText = ft;
  if (Array.isArray(components)) {
    for (const c of components) {
      ...
      for (const p of params) {
        ...
        const param = p as { type?: string; image?: { link?: string; id?: string } };
        if (param.type === "image" && param.image && typeof param.image === "object") {
          ...
          if (link) out.headerImageUrl = link;
          if (id) out.headerImageMediaId = id;
        }
      }
    }
  }
...
async function getCompanyWhatsappConfigByAuthUser(userId: string) {
  const u = await User.findById(userId).lean<LeanUser>();
  if (!u) return { error: { code: 404, message: "Usuario no encontrado" } as const };
  if (!u.companyId) return { error: { code: 400, message: "Debes crear empresa primero" } as const };
  const cfg = await CompanyWhatsappConfig.findOne({ companyId: u.companyId }).lean<LeanCompanyWhatsappConfig>();
  if (!cfg) return { error: { code: 404, message: "No existe configuración WhatsApp para esta empresa" } as const };
  return { cfg } as const;
}

async function fetchMetaTemplateById(cfg: LeanCompanyWhatsappConfig, templateId: string) {
  ...
  if (!rMeta.ok) return { ok: false as const, status: rMeta.status, data };
  return { ok: true as const, data };
}

function signToken(deps: SendApiDeps, userId: string, companyId?: string): string {
  return jwt.sign(companyId ? { companyId } : {}, deps.jwtSecret, { subject: userId, expiresIn: "7d" });
}

async function createOtp(purpose: "login" | "register", whatsappDigits: string, registerDraft?: unknown) {
...
async function verifyOtp(purpose: "login" | "register", whatsappDigits: string, code: string) {
  const now = new Date();
  ...
  if (!challenge) return { ok: false as const, error: "Código inválido o expirado" };
  if (challenge.attemptsLeft <= 0) return { ok: false as const, error: "Sin intentos disponibles" };

  const okCode = challenge.codeHash === hashOtpCode(code);
  if (!okCode) {
    challenge.attemptsLeft -= 1;
    await challenge.save();
    return { ok: false as const, error: "Código incorrecto", attemptsLeft: challenge.attemptsLeft };
  }
...
function inferPublicSendApiBase(req: { protocol?: string; secure?: boolean; headers: Record<string, string | string[] | undefined>; get(name: string): string | undefined }): string {
  ...
  const host =
    (typeof xfHostRaw === "string" ? xfHostRaw.split(",")[0].trim() : "") || req.get("host") || "localhost";
...
export function createSendApiRouter(deps: SendApiDeps): Router {
  ...
  const persistQrOutboundAndNotify = async (params: {
    ...
    type OutResp = { inserted?: boolean; message?: Record<string, unknown> };
    let data: OutResp = {};
    try {
      data = raw ? (JSON.parse(raw) as OutResp) : {};
    } catch {
      data = {};
    }
    if (!data.inserted) return;
    ...
    const io = deps.getIo?.();
    io?.to(room).emit("message:new", { channel: "qr_baileys" as const, waId, message: msg });
    io?.to(room).emit("chat:updated", {
      channel: "qr_baileys" as const,
      waId,
  ...
  r.get("/public/template-sample/:token", async (req, res) => {
    ...
    const doc = await TemplateSampleUpload.findOne({ token }).lean<{ mimeType?: string; data?: Buffer }>();
    if (!doc?.data?.length) return res.status(404).send("Not found");
  ...
  r.post("/internal/qr/inbound", async (req, res) => {
    try {
      ...
      const parsedTs =
        typeof tsRaw === "number" || typeof tsRaw === "string" ? new Date(tsRaw) : new Date();
      ...
      if (!fr.ok) return res.status(fr.status).json(data ?? { error: "qr inbox forward failed" });
      ...
      if (d.inserted) {
        ...
        io?.to(room).emit("chat:updated", {
          ...
          peerJid: typeof body.peerJid === "string" ? body.peerJid : undefined,
          lastMessagePreview:
            (typeof body.bodyText === "string" && body.bodyText.trim()) || `[${type}]`,
  ...
  r.post("/media/template-sample-hosted", auth, upload.single("file"), async (req: AuthedRequest, res) => {
    ...
    const file = req.file;
    if (!file) return res.status(400).json({ error: "file es obligatorio" });
  ...
  r.post("/auth/register/request-otp", async (req, res) => {
    try {
      ...
      if (!user.name || !user.email || !user.whatsapp) return res.status(400).json({ error: "Usuario incompleto" });

      const dupUser = await User.findOne({ $or: [{ email: user.email }, { whatsapp: user.whatsapp }] }).lean<LeanUser>();
  ...
  r.post("/auth/register/verify", async (req, res) => {
    try {
      ...
      if (!whatsapp || code.length !== 6) return res.status(400).json({ error: "Datos inválidos" });

      const v = await verifyOtp("register", whatsapp, code);
      if (!v.ok) return res.status(400).json({ error: v.error, attemptsLeft: "attemptsLeft" in v ? v.attemptsLeft : undefined });

      type RegisterDraft = {
        user: { name: string; email: string; whatsapp: string };
      };

      const draft = v.challenge.registerDraft as RegisterDraft | undefined;
      if (!draft?.user) return res.status(400).json({ error: "Sesión de registro inválida" });
  ...
  r.post("/auth/login/request-otp", async (req, res) => {
    try {
      ...
      if (!whatsapp) return res.status(400).json({ error: "WhatsApp inválido" });
      const u = await User.findOne({ whatsapp }).lean<LeanUser>();
  ...
  r.get("/auth/me", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const u = await User.findById(req.auth.userId).lean<LeanUser>();
    if (!u) return res.status(404).json({ error: "Usuario no encontrado" });
    if (!u.companyId) return res.json({ user: { id: u._id, name: u.name, email: u.email, whatsapp: u.whatsapp }, company: null, whatsappConfig: null, webhook: null });
    const c = await Company.findById(u.companyId).lean<LeanCompany>();
    if (!c) return res.json({ user: { id: u._id, name: u.name, email: u.email, whatsapp: u.whatsapp }, company: null, whatsappConfig: null, webhook: null });
    const wc = await CompanyWhatsappConfig.findOne({ companyId: c._id }).lean<LeanCompanyWhatsappConfig>();
    const webhook = await CompanyWebhookVerifyToken.findOne({ companyId: c._id }).lean<{
      _id: Types.ObjectId;
      token: string;
      verifiedAt?: Date;
      liveTestPassedAt?: Date;
    }>();
    return res.json({
      user: { id: u._id, name: u.name, email: u.email, whatsapp: u.whatsapp },
      company: {
  ...
  r.post("/company", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const u = await User.findById(req.auth.userId);
    if (!u) return res.status(404).json({ error: "Usuario no encontrado" });
    if (u.companyId) return res.status(409).json({ error: "Este usuario ya tiene empresa asociada" });
  ...
  r.put("/company", auth, async (req: AuthedRequest, res) => {
    ...
    if (!current) return res.status(404).json({ error: "Empresa no encontrada" });
    ...
    const dupCompanyNit = await Company.findOne({ nit: next.nit, _id: { $ne: current._id } }).lean<LeanCompany>();
    if (dupCompanyNit) return res.status(409).json({ error: "Ya existe una empresa con ese NIT" });
  ...
  r.get("/company/whatsapp", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const u = await User.findById(req.auth.userId).lean<LeanUser>();
    if (!u) return res.status(404).json({ error: "Usuario no encontrado" });
    if (!u.companyId) return res.status(404).json({ error: "No tienes empresa configurada" });
    const wc = await CompanyWhatsappConfig.findOne({ companyId: u.companyId }).lean<LeanCompanyWhatsappConfig>();
    if (!wc) return res.json({ whatsappConfig: null });
    return res.json({
      whatsappConfig: {
        id: wc._id,
        companyId: wc.companyId,
  ...
  r.post("/company/whatsapp", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const u = await User.findById(req.auth.userId).lean<LeanUser>();
    if (!u) return res.status(404).json({ error: "Usuario no encontrado" });
    if (!u.companyId) return res.status(400).json({ error: "Debes crear empresa primero" });

    const existing = await CompanyWhatsappConfig.findOne({ companyId: u.companyId }).lean<LeanCompanyWhatsappConfig>();
    if (existing) return res.status(409).json({ error: "La empresa ya tiene configuración WhatsApp" });
    ...
    const dupPhoneId = await CompanyWhatsappConfig.findOne({ waPhoneNumberId: payload.waPhoneNumberId }).lean<LeanCompanyWhatsappConfig>();
    if (dupPhoneId) return res.status(409).json({ error: "WA_PHONE_NUMBER_ID ya está registrado" });

    const wc = await CompanyWhatsappConfig.create(payload);
    return res.status(201).json({ whatsappConfig: wc });
  });

  r.get("/company/webhook", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const u = await User.findById(req.auth.userId).lean<LeanUser>();
    if (!u) return res.status(404).json({ error: "Usuario no encontrado" });
    if (!u.companyId) return res.status(404).json({ error: "No tienes empresa configurada" });
    ...
    if (!wh) return res.json({ webhook: null });
    return res.json({
      webhook: {
        id: wh._id,
  ...
  r.post("/company/webhook/generate-verify-token", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const u = await User.findById(req.auth.userId).lean<LeanUser>();
    if (!u) return res.status(404).json({ error: "Usuario no encontrado" });
    if (!u.companyId) return res.status(400).json({ error: "Debes crear empresa primero" });
    const token = `meta_${crypto.randomBytes(18).toString("hex")}`;
    await CompanyWebhookVerifyToken.findOneAndUpdate(
      { companyId: u.companyId },
      { $set: { token, verifiedAt: undefined }, $unset: { liveTestPassedAt: 1 } },
      { upsert: true, new: true },
    );
    return res.json({ verifyToken: token });
  });

  r.put("/company/webhook", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const u = await User.findById(req.auth.userId).lean<LeanUser>();
    if (!u) return res.status(404).json({ error: "Usuario no encontrado" });
    if (!u.companyId) return res.status(400).json({ error: "Debes crear empresa primero" });
    const verifyToken = String(req.body?.verifyToken ?? "").trim();
    if (!verifyToken) return res.status(400).json({ error: "verifyToken es obligatorio" });
    const dup = await CompanyWebhookVerifyToken.findOne({ token: verifyToken, companyId: { $ne: u.companyId } }).lean();
    if (dup) return res.status(409).json({ error: "El verify token ya está en uso" });
    const wh = await CompanyWebhookVerifyToken.findOneAndUpdate(
      { companyId: u.companyId },
      { $set: { token: verifyToken, verifiedAt: undefined }, $unset: { liveTestPassedAt: 1 } },
      { upsert: true, new: true },
    ).lean<{ _id: Types.ObjectId; token: string; verifiedAt?: Date; liveTestPassedAt?: Date }>();
    return res.json({
      webhook: {
        id: wh?._id,
        verifyToken: wh?.token,
  ...
  r.post("/company/webhook/start-live-test", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const u = await User.findById(req.auth.userId).lean<LeanUser>();
    if (!u) return res.status(404).json({ error: "Usuario no encontrado" });
    if (!u.companyId) return res.status(400).json({ error: "Debes crear empresa primero" });
    const wh = await CompanyWebhookVerifyToken.findOne({ companyId: u.companyId }).lean<{ token?: string }>();
    if (!wh?.token?.trim()) {
  ...
  r.post("/company/webhook/cancel-live-test", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const u = await User.findById(req.auth.userId).lean<LeanUser>();
    if (!u?.companyId) return res.status(400).json({ error: "Sin empresa" });
    cancelLiveTest(String(u.companyId));
    return res.json({ ok: true });
  });

  r.put("/company/whatsapp", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const u = await User.findById(req.auth.userId).lean<LeanUser>();
    if (!u) return res.status(404).json({ error: "Usuario no encontrado" });
    if (!u.companyId) return res.status(400).json({ error: "Debes crear empresa primero" });
    ...
    if (!current) return res.status(404).json({ error: "No existe configuración WhatsApp para esta empresa" });
    ...
    const dupPhoneId = await CompanyWhatsappConfig.findOne({ waPhoneNumberId: next.waPhoneNumberId, _id: { $ne: current._id } }).lean<LeanCompanyWhatsappConfig>();
    if (dupPhoneId) return res.status(409).json({ error: "WA_PHONE_NUMBER_ID ya está registrado" });
  ...
  r.delete("/company/whatsapp", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const u = await User.findById(req.auth.userId).lean<LeanUser>();
    if (!u) return res.status(404).json({ error: "Usuario no encontrado" });
    if (!u.companyId) return res.status(400).json({ error: "Debes crear empresa primero" });
  ...
  r.get("/company/messages/start-template/options", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    ...
    const { cfg } = got;
    if (!cfg.waWabaId) return res.status(400).json({ error: "Debes configurar WA_WABA_ID para consultar plantillas" });
    ...
    if (!rMeta.ok) return res.status(rMeta.status).json({ error: "Meta templates query failed", detail: data });
    const rows = Array.isArray((data as { data?: unknown[] })?.data) ? ((data as { data?: unknown[] }).data ?? []) : [];
    const options = rows
      .map((x) => {
        const o = x as { id?: unknown; name?: unknown; language?: unknown; status?: unknown; category?: unknown };
        return {
          id: String(o.id ?? "").trim(),
  ...
  r.get("/company/messages/start-template", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const got = await getCompanyWhatsappConfigByAuthUser(req.auth.userId);
    if ("error" in got) {
      const err = got.error ?? { code: 500, message: "Error interno" };
    ...
    const { cfg } = got;
    const saved = (cfg.messageStartTemplate ?? {}) as MessageStartTemplateCfg;
    const out = {
    ...
    if (!out.metaTemplateId) return res.json({ config: out, detail: null });
    const meta = await fetchMetaTemplateById(cfg, out.metaTemplateId);
    if (!meta.ok) return res.json({ config: out, detail: null });
    return res.json({ config: out, detail: meta.data });
  });

  r.put("/company/messages/start-template", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const got = await getCompanyWhatsappConfigByAuthUser(req.auth.userId);
    if ("error" in got) {
      const err = got.error ?? { code: 500, message: "Error interno" };
    ...
    const { cfg } = got;
    const b = req.body ?? {};
    const metaTemplateId = String((b as { metaTemplateId?: string }).metaTemplateId ?? "").trim();
    const templateName = String((b as { templateName?: string }).templateName ?? "").trim();
    const languageCode = String((b as { languageCode?: string }).languageCode ?? "").trim();
    ...
    if (!meta.ok) return res.status(meta.status).json({ error: "No se pudo validar la plantilla en Meta", detail: meta.data });
    await CompanyWhatsappConfig.updateOne(
      { _id: cfg._id },
      {
        $set: {
  ...
  r.get("/company/templates", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const got = await getCompanyWhatsappConfigByAuthUser(req.auth.userId);
    if ("error" in got) {
      const err = got.error ?? { code: 500, message: "Error interno" };
    ...
    const url = new URL(`https://graph.facebook.com/${encodeURIComponent(cfg.graphApiVersion)}/${encodeURIComponent(cfg.waWabaId)}/message_templates`);
    if (typeof req.query.name === "string" && req.query.name.trim()) url.searchParams.set("name", req.query.name.trim());
  ...
  r.get("/company/templates/:templateId", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const got = await getCompanyWhatsappConfigByAuthUser(req.auth.userId);
    if ("error" in got) {
      const err = got.error ?? { code: 500, message: "Error interno" };
    ...
    if (!templateId) return res.status(400).json({ error: "templateId inválido" });
    ...
    if (!rMeta.ok) return res.status(rMeta.status).json({ error: "Meta template get failed", detail: data });
    return res.json(data);
  });

  r.post("/company/templates", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const got = await getCompanyWhatsappConfigByAuthUser(req.auth.userId);
    if ("error" in got) {
      const err = got.error ?? { code: 500, message: "Error interno" };
    ...
    const { cfg } = got;
    if (!cfg.waWabaId) return res.status(400).json({ error: "Debes configurar WA_WABA_ID para crear plantillas" });
    ...
    const payload: Record<string, unknown> = { name, language, category, components };
    ...
    if (!rMeta.ok) return res.status(rMeta.status).json({ error: "Meta template create failed", detail: data });
    return res.status(rMeta.status === 201 ? 201 : 200).json(data);
  });

  r.post("/company/templates/:templateId", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const got = await getCompanyWhatsappConfigByAuthUser(req.auth.userId);
    if ("error" in got) {
      const err = got.error ?? { code: 500, message: "Error interno" };
    ...
    if (Array.isArray(b.components)) patch.components = b.components;
    if (typeof b.category === "string" && b.category.trim()) patch.category = String(b.category).trim().toUpperCase();
    if (typeof b.language === "string" && b.language.trim()) patch.language = String(b.language).trim();
    if (typeof b.name === "string" && b.name.trim()) patch.name = String(b.name).trim().toLowerCase().replace(/\s+/g, "_");
    if (typeof b.parameter_format === "string" && b.parameter_format.trim()) {
    ...
    if (Object.keys(patch).length === 0) return res.status(400).json({ error: "Envía al menos components, category, language o name para editar" });
    ...
    if (!rMeta.ok) return res.status(rMeta.status).json({ error: "Meta template edit failed", detail: data });
    return res.json(data);
  });

  r.delete("/company/templates", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const got = await getCompanyWhatsappConfigByAuthUser(req.auth.userId);
    if ("error" in got) {
      const err = got.error ?? { code: 500, message: "Error interno" };
    ...
    const { cfg } = got;
    if (!cfg.waWabaId) return res.status(400).json({ error: "Debes configurar WA_WABA_ID para eliminar plantillas" });

    const name = typeof req.query.name === "string" ? req.query.name.trim() : "";
    const hsmId = typeof req.query.hsm_id === "string" ? req.query.hsm_id.trim() : "";
    if (!name && !hsmId) return res.status(400).json({ error: "Indica name o hsm_id en query" });
    ...
    if (!rMeta.ok) return res.status(rMeta.status).json({ error: "Meta template delete failed", detail: data });
  ...
  r.post("/messages/template", auth, async (req: AuthedRequest, res) => {
    ...
    const previewText = String((b as { previewText?: string }).previewText ?? "").trim();
    const footerText = String((b as { footerText?: string }).footerText ?? "").trim();
  ...
  r.post("/messages/text", auth, async (req: AuthedRequest, res) => {
    ...
    const peerJid = String((b as { peerJid?: string }).peerJid ?? "").trim();
    ...
    if (resolveOutboundChannel(company) === "qr_baileys") {
      ...
      try {
        ...
        return res.status(500).json({
          error:
            "El mensaje pudo salir por WhatsApp, pero no se guardó en el historial QR. Revisa el servicio qr-inbox y vuelve a intentar.",
        });
      }
      console.info(
        "[qr-wa] send text ok",
    ...
    if (!rMeta.ok) return res.status(rMeta.status).json({ error: "Meta send text failed", detail: data });
    ...
    console.info(
      "[whatsapp/graph] send text ok",
      JSON.stringify(
        { companyId: String(u.companyId), to, wamid: wamid ?? null, metaResponse: data },
        null,
        0,
      ),
    );
  ...
  r.post("/messages/baileys/text", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const u = await User.findById(req.auth.userId).lean<LeanUser>();
    if (!u?.companyId) return res.status(400).json({ error: "Debes crear empresa primero" });
    const b = req.body ?? {};
  ...
  r.post("/messages/baileys/media", auth, upload.single("file"), async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const u = await User.findById(req.auth.userId).lean<LeanUser>();
    if (!u?.companyId) return res.status(400).json({ error: "Debes crear empresa primero" });
    if (!qrBase || !qrKey) {
    ...
    const file = req.file;
    if (!file?.buffer?.length) return res.status(400).json({ error: "file es obligatorio" });
    ...
    const mediaKind: "image" | "video" | "document" =
      forcedKind === "image" || forcedKind === "video" || forcedKind === "document"
        ? forcedKind
    ...
    fd.append("to", to);
    if (peerJid) fd.append("peerJid", peerJid);
    ...
    console.info(
      "[qr-wa] send baileys media ok",
      JSON.stringify({ companyId: String(u.companyId), to, wamid, mediaKind, synthetic: true }, null, 0),
    );
    return res.json({ messaging_product: "whatsapp", channel: "qr_baileys", messages: [{ id: wamid }] });
  });
  ...
  r.post("/messages/qr-image", auth, async (req: AuthedRequest, res) => {
    ...
    if (!imageUrl) return res.status(400).json({ error: "to e imageUrl son obligatorios" });
  ...
  r.post("/messages/image", auth, upload.single("file"), async (req: AuthedRequest, res) => {
    ...
    const displayName = String(req.body?.displayName ?? "").trim();
    const file = req.file;
    if (!to || !file) return res.status(400).json({ error: "to y file son obligatorios" });
    ...
    if (!uploadResp.ok) return res.status(uploadResp.status).json({ error: "Meta upload media failed", detail: uploadData });
    const mediaId = typeof uploadData === "object" && uploadData && "id" in (uploadData as object) ? String((uploadData as { id?: string }).id ?? "") : "";
    if (!mediaId) return res.status(502).json({ error: "No se recibió media id de Meta" });
    ...
    if (!rMeta.ok) return res.status(rMeta.status).json({ error: "Meta send image failed", detail: data });
    ...
    console.info(
      "[whatsapp/graph] send image ok",
      JSON.stringify(
        { companyId: String(u.companyId), to, wamid: wamid ?? null, mediaId, metaResponse: data },
        null,
        0,
      ),
    );
  ...
  r.post("/media/upload", auth, upload.single("file"), async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const got = await getCompanyWhatsappConfigByAuthUser(req.auth.userId);
    if ("error" in got) {
      const err = got.error ?? { code: 500, message: "Error interno" };
    ...
    if (!uploadResp.ok) return res.status(uploadResp.status).json({ error: "Meta media upload failed", detail: data });
    const mediaId = typeof data === "object" && data && "id" in (data as object) ? String((data as { id?: string }).id ?? "") : "";
    if (mediaId) {
   ...
   */
  r.post("/media/template-resumable", auth, upload.single("file"), async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const got = await getCompanyWhatsappConfigByAuthUser(req.auth.userId);
    if ("error" in got) {
      const err = got.error ?? { code: 500, message: "Error interno" };
    ...
    const { cfg } = got;
    const file = req.file;
    if (!file) return res.status(400).json({ error: "file es obligatorio (multipart campo file)" });
    ...
    const sessionId =
      typeof startData === "object" && startData && "id" in (startData as object)
        ? String((startData as { id?: string }).id ?? "")
        : "";
    if (!sessionId) return res.status(502).json({ error: "Meta no devolvió id de sesión de subida", detail: startData });
    ...
    const handle =
      typeof uploadData === "object" && uploadData && "h" in (uploadData as object)
        ? String((uploadData as { h?: string }).h ?? "")
        : "";
    if (!handle) return res.status(502).json({ error: "Meta no devolvió el handle de muestra (campo h)", detail: uploadData });

    return res.json({ handle });
  });

  r.get("/media/uploads", auth, async (req: AuthedRequest, res) => {
  ...
  r.get("/media/:mediaId", auth, async (req: AuthedRequest, res) => {
    if (!req.auth) return res.status(401).json({ error: "No autenticado" });
    const got = await getCompanyWhatsappConfigByAuthUser(req.auth.userId);
    if ("error" in got) {
      const err = got.error ?? { code: 500, message: "Error interno" };
    ...
    if (!mediaId) return res.status(400).json({ error: "mediaId inválido" });
    ...
    let metaInfo: unknown = null;
    try {
      metaInfo = metaInfoText ? JSON.parse(metaInfoText) : null;
    } catch {
      metaInfo = { raw: metaInfoText };
    }
    if (!metaInfoResp.ok) return res.status(metaInfoResp.status).json({ error: "Meta media info failed", detail: metaInfo });

    const mediaUrl = typeof metaInfo === "object" && metaInfo && "url" in (metaInfo as object) ? String((metaInfo as { url?: string }).url ?? "") : "";
    if (!mediaUrl) return res.status(404).json({ error: "No se pudo resolver URL de media" });
    ...
    const contentTypeFromMetaInfo =
      typeof metaInfo === "object" && metaInfo && "mime_type" in (metaInfo as object) ? String((metaInfo as { mime_type?: string }).mime_type ?? "") : "";
  ...
  r.get("/clients", auth, async (req: AuthedRequest, res) => {
    if (!req.auth?.companyId) return res.status(403).json({ error: "Debes configurar tu empresa primero" });
    const rows = await Client.find({ companyId: req.auth.companyId }).sort({ updatedAt: -1 }).lean();
    return res.json(rows);
  });

  r.post("/clients", auth, async (req: AuthedRequest, res) => {
    if (!req.auth?.companyId) return res.status(403).json({ error: "Debes configurar tu empresa primero" });
    const b = req.body ?? {};
    const name = String(b.name ?? "").trim();
  ...
  r.patch("/clients/:id", auth, async (req: AuthedRequest, res) => {
    if (!req.auth?.companyId) return res.status(403).json({ error: "Debes configurar tu empresa primero" });
    ...
    if (typeof b.email === "string") patch.email = b.email.trim().toLowerCase() || undefined;
    if (typeof b.phone === "string") patch.phone = normalizeDigits(b.phone);
    if (typeof b.companyName === "string") patch.companyName = b.companyName.trim() || undefined;
  ...
  r.delete("/clients/:id", auth, async (req: AuthedRequest, res) => {
    if (!req.auth?.companyId) return res.status(403).json({ error: "Debes configurar tu empresa primero" });
    const id = req.params.id;
    if (!isValidObjectId(id)) return res.status(400).json({ error: "ID inválido" });
    const r0 = await Client.deleteOne({ _id: id, companyId: req.auth.companyId });
    if (!r0.deletedCount) return res.status(404).json({ error: "Cliente no encontrado" });
    return res.json({ ok: true });
  });

  r.post("/massivity/campaigns", auth, async (req: AuthedRequest, res) => {
    if (!req.auth?.companyId) return res.status(403).json({ error: "Debes configurar tu empresa primero" });
    const b = req.body ?? {};
    const name = String((b as { name?: string }).name ?? "").trim();
    const fileName = String((b as { fileName?: string }).fileName ?? "").trim();
    const phoneColumn = String((b as { phoneColumn?: string }).phoneColumn ?? "").trim();
    const templateName = String((b as { templateName?: string }).templateName ?? "").trim();
    const languageCode = String((b as { languageCode?: string }).languageCode ?? "").trim();
    const intervalSec = Math.max(1, Number((b as { intervalSec?: number }).intervalSec) || 1);
    const rowCount = Math.max(0, Number((b as { rowCount?: number }).rowCount) || 0);
    const variableMapping = (b as { variableMapping?: unknown }).variableMapping ?? {};
    const headerImageModeRaw = String((b as { headerImageMode?: string }).headerImageMode ?? "").trim();
    const headerImageMode = headerImageModeRaw === "url" || headerImageModeRaw === "mediaId" ? headerImageModeRaw : undefined;
    const headerImageUrl = String((b as { headerImageUrl?: string }).headerImageUrl ?? "").trim();
    const headerImageMediaId = String((b as { headerImageMediaId?: string }).headerImageMediaId ?? "").trim();
  ...
  r.get("/massivity/campaigns/:id", auth, async (req: AuthedRequest, res) => {
    if (!req.auth?.companyId) return res.status(403).json({ error: "Debes configurar tu empresa primero" });
  ...
  r.patch("/massivity/campaigns/:id/status", auth, async (req: AuthedRequest, res) => {
    if (!req.auth?.companyId) return res.status(403).json({ error: "Debes configurar tu empresa primero" });
    const id = String(req.params.id ?? "");
    if (!isValidObjectId(id)) return res.status(400).json({ error: "ID inválido" });
    const status = String((req.body as { status?: string } | undefined)?.status ?? "").trim();
    if (!["draft", "sent"].includes(status)) return res.status(400).json({ error: "status inválido" });
    const sentCountBody = Math.max(0, Number((req.body as { sentCount?: number } | undefined)?.sentCount) || 0);
    const failCountBody = Math.max(0, Number((req.body as { failCount?: number } | undefined)?.failCount) || 0);
    const rawRowResults = (req.body as { rowResults?: unknown }).rowResults;
    let rowResults: Record<string, unknown>[] | undefined;
    if (Array.isArray(rawRowResults)) {
      ...
      for (const item of rawRowResults) {
        ...
        const reason = String(o.reason ?? "")
          .trim()
          .slice(0, 500);
        const row: Record<string, unknown> = { rowIndex, phone };
        if (wamid) row.wamid = wamid;
        if (typeof o.apiOk === "boolean") row.apiOk = o.apiOk;
        if (deliveryStatus) row.deliveryStatus = deliveryStatus;
        if (typeof o.ok === "boolean") row.ok = o.ok;
        if (reason) row.reason = reason;
        const dua = o.deliveryUpdatedAt;
        if (dua) {
          const d = new Date(String(dua));
          if (!Number.isNaN(d.getTime())) row.deliveryUpdatedAt = d;
        }
        normalized.push(row);
        if (normalized.length >= 50_000) break;
      }
    ...
    if (rowResults !== undefined) {
      ...
      mergedRowResults = mergeIncomingRowResultsWithExisting(
        ...
        rowResults as Record<string, unknown>[],
      );
    }
  ...
  r.post("/qr/messages/text", auth, async (req: AuthedRequest, res) => {
    ...
    if (!qrBase || !qrKey) return res.status(501).json({ error: "Canal QR no configurado" });
    try {
      const b = (req.body ?? {}) as { to?: string; text?: string; peerJid?: string };
      const to = canonicalWaId(normalizeDigits(b.to ?? ""));
