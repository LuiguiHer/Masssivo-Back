...
function httpToWsBase(url: string): string {
 ...
 */
export function attachQrWaWebSocketProxy(httpServer: HttpServer, jwtSecret: string): void {
  ...
  wss.on("connection", (clientWs, req) => {
    ...
    try {
      const pl = jwt.verify(token, jwtSecret) as { sub?: string; companyId?: string };
      if (!pl?.companyId) {
    ...
    upstream.on("open", () => {
      const pipe = (from: WebSocket, to: WebSocket) => {
        from.on("message", (d, isBinary) => {
          if (to.readyState === WebSocket.OPEN) to.send(d, { binary: isBinary });
        });
      };
