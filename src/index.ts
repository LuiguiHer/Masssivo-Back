...
const app = createApp({
  ...
  onWebhookVerifySuccess: async (token) => {
    const tokenTrim = token.trim();
    const now = new Date();
    const hit = (await CompanyWebhookVerifyToken.findOneAndUpdate({ token: tokenTrim }, { $set: { verifiedAt: now } }, { new: true }).lean()) as {
...
io.use((socket, next) => {
  try {
    ...
    if (typeof payload === "string" || !payload.sub) return next(new Error("unauthorized"));
    const companyId = "companyId" in payload ? payload.companyId : undefined;
    if (!companyId) return next(new Error("unauthorized"));
